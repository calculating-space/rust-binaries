...
pub fn validate_name(name: &str) -> Result<(), String> {
    let ok = !name.is_empty()
        ...
        && !name.starts_with('-');
    if ok { Ok(()) } else { Err(format!("invalid sandbox name {name:?}: use [A-Za-z0-9_-], max 64 chars")) }
}
...
pub fn overview(all: &[Manifest]) -> String {
    ...
    for m in all {
        ...
        let (summary, what, recipe_name) = match recipe(&m.recipe) {
            ...
            Err(_) => (p.summary(0), format!("unknown recipe {}", m.recipe), m.recipe.as_str()),
        };
        let last = ago(p.last_unix().unwrap_or(m.created_unix));
        out.push_str(&format!("{}\n", m.name));
        out.push_str(&recipe::wrap(&what, 4, 78));
        out.push_str(&format!("    {summary}  ·  {}  ·  last used {last}\n", gb(backend::dir_size(&m.dir))));
        for (cmd, what) in [(format!("trybox explore {recipe_name}"), "continue"), (format!("trybox status {}", m.name), "details"), (format!("trybox dispose {} -y", m.name), "free the disk")] {
...
pub fn status(m: &Manifest) -> String {
    ...
    match recipe(&m.recipe) {
        Ok(r) => {
            out.push_str(&format!("  what      {}\n  progress  {}\n", r.summary, p.summary(r.tour.len())));
            out.push_str(&format!("  hello     {}\n", match &p.hello { Some(run) if run.ok => format!("ok, {}", ago(run.at_unix)), Some(run) => format!("failed, {}", ago(run.at_unix)), None => "not run".into() }));
            for (i, e) in r.tour.iter().enumerate() {
                let mark = match p.steps.get(&i) { Some(run) if run.ok => format!("✓ {}", ago(run.at_unix)), Some(run) => format!("✗ {}", ago(run.at_unix)), None => "·".into() };
        ...
        Err(_) => out.push_str(&format!("  recipe    {} (not built into this trybox)\n", m.recipe)),
    }
    out.push_str(&format!("  size      {} total, {} of it models\n", gb(backend::dir_size(&m.dir)), gb(backend::dir_size(&m.hf_dir()))));
