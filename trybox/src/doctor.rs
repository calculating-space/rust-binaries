...
pub fn host_info() -> HostInfo {
    ...
    let (chip, memory_gb) = if os == "macos" {
        ...
        let mem = sysctl("hw.memsize").and_then(|s| s.parse::<u64>().ok()).map(|b| b >> 30).unwrap_or(0);
        (chip, mem)
    } else {
    ...
    HostInfo { os, arch, chip, memory_gb }
}
...
fn version_of(bin: &str, args: &[&str]) -> ToolStatus {
    match Command::new(bin).args(args).output() {
        Ok(out) if out.status.success() => {
            ...
            let text = if text.trim().is_empty() { String::from_utf8_lossy(&out.stderr) } else { text };
            ToolStatus { name: bin.into(), available: true, detail: text.lines().next().unwrap_or("").trim().to_string() }
        }
        Ok(out) => ToolStatus { name: bin.into(), available: false, detail: format!("exit {}", out.status) },
        Err(e) => ToolStatus { name: bin.into(), available: false, detail: e.to_string() },
    }
}
...
pub fn doctor() -> Report {
    let host = host_info();
    let mut tools = vec![version_of("uv", &["--version"]), version_of("python3", &["--version"]), version_of("docker", &["--version"]), version_of("claude", &["--version"])];
    let daemon = Command::new("docker").args(["info", "--format", "{{.ServerVersion}}"]).output();
    let daemon_up = matches!(&daemon, Ok(o) if o.status.success());
    tools.push(ToolStatus {
        name: "docker-daemon".into(),
        available: daemon_up,
        detail: if daemon_up { "running".into() } else { "not running (start Docker Desktop to use --backend docker)".into() },
    });
