...
pub fn choices(all: &[Manifest]) -> (Vec<Choice>, Vec<Action>) {
    let mut rows: Vec<(&Manifest, Progress, u64)> = all
        .iter()
        .map(|m| (m, Progress::load(&m.dir), backend::dir_size(&m.dir)))
        .collect();
    ...
    for (i, (m, p, size)) in rows.iter().enumerate() {
        ...
        let c = Choice::new(
            format!("Continue exploring {}", m.name),
            format!(
                "{what} · {progress} · {} · last used {}",
                gb(*size),
                ago(p.last_unix().unwrap_or(m.created_unix))
            ),
        );
        choices.push(if i == 0 { c.recommended() } else { c });
        actions.push(Action::Continue(m.name.clone()));
    }
    let start = Choice::new(
        "Start something new",
        "Pick a recipe: a guided tour of one project",
    );
    choices.push(if rows.is_empty() {
        start.recommended()
    } else {
        start
    });
    ...
    for (m, _, size) in &rows {
        choices.push(Choice::new(
            format!("Dispose {}", m.name),
            format!(
                "Delete the sandbox and everything it downloaded, freeing {}",
                gb(*size)
            ),
        ));
...
pub fn recipe_choices(
    root: &Path,
    existing: &[Manifest],
) -> Vec<(Choice, &'static Recipe, Outcome)> {
    let mut out = Vec::new();
    ...
    for r in RECIPES {
        ...
        let verdict = match v.outcome {
            ...
            Outcome::Pointless => {
                let why = v
                    .findings
                    .iter()
                    .find(|f| !f.pass && f.severity == speccheck::Severity::Pointless)
                    .map(|f| f.why.as_str())
                    .unwrap_or("");
            ...
            Outcome::CannotRun => {
                let why = v
                    .findings
                    .iter()
                    .find(|f| !f.pass && f.severity == speccheck::Severity::Blocks)
                    .map(|f| f.why.as_str())
                    .unwrap_or("");
        ...
        let mut c = Choice::new(
            label,
            format!("{} · {} steps · {verdict}", r.summary, r.tour.len() + 1),
        );
...
pub fn run(root: &Path) -> Result<Option<Manifest>, String> {
    loop {
        ...
        let Some(pick) = select("trybox: what do you want to do?", &choices)? else {
            return Ok(None);
        };
        match &actions[pick] {
            ...
            Action::Dispose(name) => {
                let m = crate::manifest::load(root, name)?;
                if confirm(
                    &format!("Dispose {name} and free {}?", gb(backend::dir_size(&m.dir))),
                    "Yes, dispose it",
                    "No, keep it",
                )? {
            ...
            Action::StartNew => {
                ...
                let Some(pick) = select("Which project do you want to explore?", &choices)? else {
                    continue;
                };
                ...
                match outcome {
                    ...
                    Outcome::Pointless => {
                        ...
                        if !confirm(
                            "Continue anyway?",
                            "Yes, I know it will run on the CPU",
                            "No, go back",
                        )? {
                            continue;
                        }
