...
Run these from a shell. Each is idempotent; skip what is already there.
...
Or run `sh bootstrap.sh` from the clone, which does the same with one status
line per step. Keep the clone at `~/rust-binaries`: `cs` records the path at
build time and stops working if the directory moves.

## Verify

```sh
cs trybox doctor            # a table of recipes with can run / cannot run and the reason
cs trybox check whisper     # exit 0 means the whisper tour will run here
cs trybox recipes           # four recipes: mlx, whisper, torch, bare
```
...
- Do not start the whisper hello world yourself. It is an interactive prompt
  ...
  When `check whisper` passes, tell them to run `cs trybox`, pick *Start
  something new*, then *whisper*, and hand the terminal back.
- Everything a tour downloads lives under `~/.trybox/<name>`; disposing from
  the menu, or `cs trybox dispose <name> -y`, removes it.

## When something fails

- `cs trybox check <recipe>` prints the requirements table with the reason.
- `cs trybox create <name> --recipe <recipe>` shows the full installer output
  that the menu hides.
- `cs trybox doctor --json` gives the same report as data.
